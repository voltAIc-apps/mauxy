...
Base URL: `https://unsubscribe.engage.wapsol.de`
...
**CORS:** Restricted to configured origins (default `simplify-erp.de`).
...
```bash
curl -X POST https://unsubscribe.engage.wapsol.de/api/unsubscribe \
...
```js
await fetch("https://unsubscribe.engage.wapsol.de/api/unsubscribe", {
  method: "POST",
  ...
  "actions": [
    {
      ...
      "email": "user@example.com",
      "source_origin": "https://simplify-erp.de",
...
```bash
curl https://unsubscribe.engage.wapsol.de/api/actions?limit=10 \
